...
class Upload extends AbstractModel
{
    ...
    public function save()
    {
      ...
      Logger::getInstance("uploading.log")->debug(
        'save-file',
        ...
        $this->emailcustomer($order, $type);

        Logger::getInstance("uploadingemail.log")->debug(
          'save-file',
          ['start']
      );
    ...
    ///function email after upload
    public function emailcustomer($order, $type){

      Logger::getInstance("uploademail.log")->debug(
        'emails',
        ['start']
    );


        $sql = 'select ru.email from recyc_order_information  r
        join recyc_customer_links_to_company rcltc on
            r.company_id = rcltc.company_id
        join recyc_users ru on rcltc.user_id = ru.id
        WHERE r.id = :order';
        $query = $this->rdb->prepare($sql);
        ...
        foreach($data as $val){
            ...
            try{
                ...
                $mail->setSubject("Documents now avaliable for collection - ".$order."  type:".$type);
                $mail->addTo(rtrim($emailadd));
                $mail->addTo('stoneitad@stonegroup.co.uk');
                            ...
                            <td class="wrapper">
                              <table border="0" cellpadding="0" cellspacing="0">
                                <tr>
                                  <td>
                                    <p>Hi,</p>
                                    <p>  Documents are now avaiable for you collection. <br> please <a href="https://itadsystem.stonegroup.co.uk/">login here</a> to portal to view </p>
                                    </td>
                ...
                if ($response->statusCode() !== 202) {
                    Logger::getInstance("uploademail.log")->error(
                        'email',
                        [
                            'line' => __LINE__,
