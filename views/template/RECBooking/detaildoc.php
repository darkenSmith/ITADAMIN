...
<ul class="nav nav-tabs">
    ...
    <h3> <?php echo $_GET['rowid']; ?> </h3>    <div id = 'button'>
...
$stmtnote->execute();
        ...
        $stmtord = $this->sdb->prepare($sqlord);
        // if (!$stmtord) {
        //   echo "\nPDO::errorInfo():\n";
        //   print_r($this->sdb->errorInfo());
        //   die();
  ...
  // $ordstmtup = $this->sdb->prepare($sqlord);
 ...
 // $ec = $stmtbook2->fetch(PDO::FETCH_ASSOC);
        ...
        foreach($data as $row) {
          ...
          $portstmt = $this->rdb->prepare($portalcheck);
          $portstmt->execute();
...
$stmtowners = $this->sdb->prepare($sqlowners);
if (!$stmtnote) {
  echo "\nPDO::errorInfo():\n";
  print_r($this->sdb->errorInfo());
  die();
...
$stmtaps = $this->sdb->prepare($sqlaps);
if( $stmtaps === false) {
...
//         $stmtbook = $this->sdb->prepare($booksql);
...
//         $stmtbook2 = $this->sdb->prepare($booksql2);
...
//         // $stmtbook2 = sqlsrv_query( $this->sdb, $booksql2);
//         // if( $stmtbook2 === false) {
...
<!-------------------------------Upload images----------------------------------------------->
          ...
          if(isset($_GET['btn'])){
            ...
            $stmtp = $this->sdb->prepare($sqlUpdate);
            $stmtp->execute();
            ...
            $st = $this->sdb->prepare($CHECKIFTHERESQL);
            $st->execute();
            ...
            $stmt = $this->sdb->prepare($bookrecord);
            $stmt->execute();
            ...
            echo "<meta http-equiv='refresh' content='0'>";
              ...
              ";
              $bookupstmt = $this->sdb->prepare($sqlbookup);
              $bookupstmt->execute();
            }
            if ($stmtp === false) {
              // failed
